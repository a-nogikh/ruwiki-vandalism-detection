...
def draw_hists(id, good, bad):
    ...
    normed = True
    axarr[id].hist(good, bins, alpha=0.5, normed=normed,color="green")
    axarr[id].hist(bad, bins, alpha=0.5, normed=normed,color="red")
...
def load_features(collection, vandal, good):
    for raw in mongo.wiki[collection].find({}):
        ...
        if (raw["revs"][-1]["text"] is None) or raw["revs"][-1]["text"] == "":
            continue

        if "bigram_stemmed" not in raw:
            continue
...
load_features('manual_new', test_vandal, test_good) #manual_new,manual_dataset
